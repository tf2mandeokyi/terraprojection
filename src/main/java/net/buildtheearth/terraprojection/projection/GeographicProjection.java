...
import net.buildtheearth.terraprojection.config.ProjectionRegistries;
import net.buildtheearth.terraprojection.config.TypedJsonAdapter;
 ...
 *
 */
@JsonAdapter(GeographicProjection.ProjectionJsonAdapter.class)
public interface GeographicProjection {
    static GeographicProjection parse(@NonNull String config) {
        return TerraProjectionConstants.GSON.fromJson(config, GeographicProjection.class);
    }
    ...
    default double[] vector(double x, double y, double north, double east) throws OutOfProjectionBoundsException {
        ...
        double[] off = this.fromGeo(geo[0] + east * 360.0 / (Math.cos(Math.toRadians(geo[1])) * TerraProjectionConstants.EARTH_CIRCUMFERENCE),
                geo[1] + north * 360.0 / TerraProjectionConstants.EARTH_POLAR_CIRCUMFERENCE);
    ...
    default double[] tissot(double longitude, double latitude, double d) throws OutOfProjectionBoundsException {
        ...
        double R = TerraProjectionConstants.EARTH_CIRCUMFERENCE / (2 * Math.PI);
    ...
    class ProjectionJsonAdapter extends TypedJsonAdapter<GeographicProjection> {
        @Override
        protected Map<String, Class<? extends GeographicProjection>> nameToClassRegistry() {
            return ProjectionRegistries.PROJECTIONS;
        }

        @Override
        protected Map<Class<? extends GeographicProjection>, String> classToNameRegistry() {
            return ProjectionRegistries.PROJECTIONS.inverse();
        }
    }
}